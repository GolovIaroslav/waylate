...
- [x] **1a. "auto" в LLM промпте** (`src-tauri/src/translation.rs:85`)
...
- [x] **1b. translate() guard не видит spec-catalog модели** (`src/routes/+page.svelte:1073`)
...
- [x] **1c. Неполная миграция model_id** (`src-tauri/src/config.rs:142`)
...
- [x] **2a. Float16 KV-cache tensor → GPU перевод всегда падает** (`src-tauri/src/engines/onnx_mt.rs:619`)
...
- [x] **2b. GPU download не атомарный** (`src-tauri/src/gpu_runtime.rs:121`)
...
- [x] **2c. GPU download нельзя отменить** (`src/routes/+page.svelte:991`)
...
- [x] **2d. Truncated fp16 файл принимается как готовый** (`src-tauri/src/gpu_runtime.rs:158`)
...
- [x] **3a. Non-atomic config save** (`src-tauri/src/config.rs:135`)
...
- [x] **3b. Corrupt config.json → hard error без fallback** (`src-tauri/src/config.rs:128`)
...
- [x] **3c. secrets::set("") записывает пустую запись в keyring** (`src-tauri/src/secrets.rs:18`)
...
- [x] **3d. history::list() принимает отрицательный limit** (`src-tauri/src/history.rs:57`)
...
- [x] **4a. context_size всегда 4096 для spec managed-llama** (`src-tauri/src/translation.rs:119`)
...
- [x] **4b. api_provider_enabled не проверяется для OpenAiCompatible** (`src-tauri/src/translation.rs:166`)
...
- [x] **4c. MODEL_CACHE Mutex удерживается на весь inference** (`src-tauri/src/engines/onnx_mt.rs:145`)
...
- [x] **4d. TranslateGemma prompt format деградировал** (`src-tauri/src/translation.rs:93`)
...
- [x] **5a. reinstallModel TOCTOU гонка** (`src/routes/+page.svelte:1276`)
...
- [x] **5b. copyTranslation() нет try/catch** (`src/routes/+page.svelte:1121`)
...
- [x] **5c. LLAMA_SERVER_RELEASE не используется в URL** (`src-tauri/src/runtime.rs:730`)
...
- [x] **5d. configure_ort_dylib без OnceLock** (`src-tauri/src/engines/onnx_mt.rs:81`)
...
- [x] **5e. WAYLATE_GPU_REEXEC — magic string без константы** (`src-tauri/src/lib.rs:1687`)
...
| Сессия | Тема | Статус |
|--------|------|--------|
| 1 | Translation: auto в промпте, guard, миграция | ✅ готово |
| 2 | GPU: Float16, атомарность, cancel, truncation | ✅ готово |
| 3 | Data safety: config, secrets, history | ✅ готово |
| 4 | Translation quality: context, network guard, Mutex | ✅ готово |
| 5 | UI races, bandaids, maintenance | ✅ готово |
